const DEFAULT_ENDPOINT = 'https://mindquest-n8n.cloudfy.live/webhook/perfil-pessoal';
...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    ...
    const remoteEndpoint = process.env.PERFIL_PESSOAL_WEBHOOK_URL || DEFAULT_ENDPOINT;
    ...
    try {
      ...
      console.error('[perfil-pessoal] Erro ao buscar perfil:', error);
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('[perfil-pessoal] Erro ao salvar perfil:', error);
      res.status(500).json({ success: false, error: 'Erro ao conectar ao serviço' });
    }
    return;
  }
