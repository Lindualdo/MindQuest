const DEFAULT_ENDPOINT = 'https://mindquest-n8n.cloudfy.live/webhook/notificacoes';
...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    ...
    const remoteEndpoint = process.env.NOTIFICACOES_WEBHOOK_URL || DEFAULT_ENDPOINT;
    ...
    try {
      ...
      console.error('[notificacoes] Erro ao buscar notificações:', error);
      res.status(500).json({ success: false, error: 'Erro ao conectar ao serviço' });
    }
    return;
  }

  // POST: Salvar notificações
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('[notificacoes] Erro ao salvar notificações:', error);
      res.status(500).json({ success: false, error: 'Erro ao conectar ao serviço' });
    }
    return;
  }
