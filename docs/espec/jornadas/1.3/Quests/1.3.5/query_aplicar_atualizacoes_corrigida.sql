-- Query corrigida para o nó "Aplicar Atualizacoes" do workflow sw_xp_quest
-- Data: 2025-01-22
...
alvos_enriquecidos AS (
  SELECT
    ...
    CASE WHEN alvos.novo_status = 'concluida' THEN COALESCE(qc.xp, 10) ELSE 0 END AS xp_base_evento,
    ...
    (
      SELECT COUNT(*) FROM jsonb_array_elements(alvos.recorrencias->'dias') AS dia_elem
    ) AS total_planejado,
    (
      SELECT COALESCE(SUM((detalhes->>'total_concluidas')::int), 0)
      FROM public.conquistas_historico hq
      WHERE hq.usuario_id = alvos.usuario_id
        AND hq.meta_codigo = alvos.id::uuid
        AND hq.tipo = 'quest'
    ) AS total_concluidas
  FROM alvos, payload
...
atualizados AS (
  ...
  SET
    ...
    quest_estagio = CASE
      WHEN ae.novo_status = 'concluida' AND (ae.total_planejado IS NULL OR ae.total_planejado <= (ae.total_concluidas + 1)) THEN 'feito'
...
historico_atualizado AS (
  ...
  WHERE ch.usuario_id = upd.usuario_id
    AND ch.meta_codigo = upd.id::uuid
    AND ch.tipo = 'quest'
